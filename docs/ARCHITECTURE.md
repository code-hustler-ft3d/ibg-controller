...
### 5. Gateway launch via install4j launcher, with `INSTALL4J_ADD_VM_PARAMS`
...
install4j and reads the `INSTALL4J_ADD_VM_PARAMS` environment variable
to get extra JVM arguments. The controller sets it to:
...
- **`-DjtsConfigDir=`** overrides Gateway's config dir. The
  install4j-bundled launcher has a literal unsubstituted
  `${installer:jtsConfigDir}` placeholder that otherwise defaults to
  `/root/Jts`, which fails for the non-root `ibgateway` user.
